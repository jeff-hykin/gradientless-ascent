...
class GaussVDSampler(StatisticalModelSamplerWithZeroMeanBaseClass):
    ...
    def update(self, vectors, weights, hsig=True):
        ...
        # Cumulation
        self.pc = (1.0 - cc) * self.pc + hsig * math.sqrt(cc * (2.0 - cc) * mueff) * np.dot(w, sary)

        # Alpha and related variables
        alpha, avec, bsca, invavnn = self._alpha_avec_bsca_invavnn(self.vnn, self.norm_v2)
        # Rank-mu
        if cmu == 0:
            pvec_mu = np.zeros(self.N)
            qvec_mu = np.zeros(self.N)
        else:
            pvec_mu, qvec_mu = self._pvec_and_qvec(self.vn, self.norm_v2, sary / self.dvec, w)
        # Rank-one
        if cone == 0:
            pvec_one = np.zeros(self.N)
            qvec_one = np.zeros(self.N)
        else:
            pvec_one, qvec_one = self._pvec_and_qvec(self.vn, self.norm_v2, self.pc / self.dvec)
        ...
        # Natural gradient
        if cmu + cone > 0:
            ngv, ngd = self._ngv_ngd(self.dvec, self.vn, self.vnn, self.norm_v, self.norm_v2, alpha, avec, bsca, invavnn, pvec, qvec)
    ...
    def _alpha_avec_bsca_invavnn(vnn, norm_v2):
        ...
        alpha = math.sqrt(norm_v2**2 + (1.0 + norm_v2) / max(vnn) * (2.0 - gamma)) / (2.0 + norm_v2)
    ...
    def _pvec_and_qvec(vn, norm_v2, y, weights=0):
        ...
        if isinstance(weights, int) and weights == 0:
            ...
            qvec = np.dot(weights, (y_vn * y.T).T - np.outer((y_vn**2 + 1.0 + norm_v2) / 2.0, vn))
        return pvec, qvec
    ...
    def _get_params2(self, mueff, **kwargs):
        ...
        cc = kwargs.get("cc", (4.0 + mueff / self.N) / (self.N + 4.0 + 2.0 * mueff / self.N))
        cone = kwargs.get("cone", cfactor * 2.0 / ((self.N + 1.3) ** 2 + mueff))
        cmu = kwargs.get("cmu", min(1.0 - cone, cfactor * 2 * (mueff - 2.0 + 1.0 / mueff) / ((self.N + 2.0) ** 2 + mueff)))
        return cc, cone, cmu
    ...
    def transform(self, x):
        ...
        y = self.dvec * (x + (math.sqrt(1.0 + self.norm_v2) - 1.0) * np.dot(x, self.vn) * self.vn)
        return y
...
class GaussVkDSampler(StatisticalModelSamplerWithZeroMeanBaseClass):
    ...
    def extend_cma_options(opts=None):
        ...
        """
        opts = opts or {}
        opts.update({"CMA_active": False, "AdaptSigma": False, "CMA_sampler": GaussVkDSampler})
        return opts
    ...
    def __init__(self, dimension, randn=np.random.randn, kadapt=True, **kwargs):
        ...
        if self.kadapt:
            ...
            self.factor_diag_slope = kwargs.get("factor_diag_slope", 2)  # 0.3 in PPSN (due to cc change)
    ...
    def sample(self, number, update=None):
        ...
        if self.flg_injection:
            ...
            X = np.asarray([dy, -dy] + [self.transform(self.randn(self.N)) for i in range(number - 2)])
        else:
            X = np.asarray([self.transform(self.randn(self.N)) for i in range(number)])
        return X
    ...
    def update(self, vectors, weights):
        ...
        if self.kadapt and not hasattr(self, "opt_conv"):
            ...
            self.ema_log_sigma = ExponentialMovingAverage(decay=self.opt_conv / self.accepted_slowdown, dim=1)
            self.ema_log_d = ExponentialMovingAverage(decay=self.k_adapt_decay, dim=self.N)
            self.ema_log_s = ExponentialMovingAverage(decay=self.k_adapt_decay, dim=self.N)
            self.itr_after_k_inc = 0

        # TPA (PPSN 2014 version
        if self.flg_injection:
            nlist = np.asarray([np.array(vectors[idx[i] + 1]) / np.linalg.norm(vectors[idx[i] + 1]) for i in range(lam)])
        ...
        # Cumulation
        self.pc = (1.0 - cc) * self.pc + hsig * math.sqrt(cc * (2.0 - cc) * mueff) * self.dx
        ...
        self.D *= np.sqrt((alpha * alpha + np.sum(self.U[:, :rankU] * self.U[:, :rankU], axis=1)) / (1.0 + np.dot(self.S[:ka], self.V[:ka] * self.V[:ka])))
        ...
        self.ema_log_d.update(2.0 * np.log(self.D) + np.log(1 + np.dot(self.S[: self.k], self.V[: self.k] ** 2)) - self.last_log_d)
        self.lndiag_change = self.ema_log_d.M / (cmu + cone)
        self.last_log_d = 2.0 * np.log(self.D) + np.log(1 + np.dot(self.S[: self.k], self.V[: self.k] ** 2))
        ...
        if (self.itr_after_k_inc > self.k_adapt_wait) and flg_k_increase:
            # ----- Increasing k -----
            self.k_active = k
            self.k = newk = min(max(int(math.ceil(self.k * self.k_adapt_factor)), self.k + 1), self.kmax)
    ...
    def _get_params2(self, mueff, k):
        ...
        cc = math.sqrt(cone)
        cmu = min(1 - cone, 2.0 * (mueff - 2 + 1.0 / mueff) / (nelem + 4 * (k + 2) + mueff))
        return cc, cone, cmu
    ...
    def variances(self):
        ...
        if ka == 0:
            ...
            return self.D**2 * (1.0 + np.dot(self.S[:ka], self.V[:ka] ** 2)) * self.sigma**2
    ...
    def transform(self, x):
        ...
        ka = self.k_active
        y = x + np.dot(np.dot(x, self.V[:ka].T) * (np.sqrt(1.0 + self.S[:ka]) - 1.0), self.V[:ka])
        y *= self.D * self.sigma
        return y
    ...
    def transform_inverse(self, x):
        ...
        if self.k_active == 0:
            ...
            return y + np.dot(np.dot(self.V[: self.k_active], y) * (1.0 / np.sqrt(1.0 + self.S[: self.k_active]) - 1.0), self.V[: self.k_active])
    ...
    def _get_log_determinant_of_cov(self):
        return 2.0 * np.sum(np.log(self.D)) + np.sum(np.log(1.0 + self.S[: self.k_active]))
