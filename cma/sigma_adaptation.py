...
class CMAAdaptSigmaBase(object):
    ...
    def _update_ps(self, es):
        ...
        try:
            if es.countiter <= es.sm.itereigenupdated:
                ...
                utils.print_warning("distribution transformation (B and D) have been updated before ps could be computed", "_update_ps", "CMAAdaptSigmaBase", verbose=es.opts["verbose"])
    ...
    def hsig(self, es):
        ...
        self._update_ps(es)
        try:
            pc_for_ps = "pc for ps" in es.opts["vv"]  # just in case
        except:
            pc_for_ps = False  # 'vv' has an incompatible format or does't exist
        if pc_for_ps:
...
class CMAAdaptSigmaCSA(CMAAdaptSigmaBase):
    ...
    def initialize(self, es):
        ...
        self.damps = es.opts["CSA_dampfac"] * (0.5 + 0.5 * min([1, (es.sp.lam_mirr / (0.159 * es.sp.popsize) - 1) ** 2]) ** 1 + 2 * max([0, ((es.sp.weights.mueff - 1) / (es.N + 1)) ** es.opts["CSA_damp_mueff_exponent"] - 1]) + self.cs)
    ...
    def update2(self, es, **kwargs):
        ...
        try:  # to filter coordinates or a
            ...
            if 11 < 3 and len(es.opts["integer_variables"]):
                m = "Missing ``path_for_sigma_update`` attribute in {}." "\n This is usually not a problem unless integer mutations are used." "".format(type(es))
                _warnings.warn(m)
        N = len(p)
        ...
        # "error" handling
        if s_clipped != s:
            utils.print_warning("sigma change np.exp(" + str(s) + ") = " + str(np.exp(s)) + " clipped to np.exp(+-" + str(self.max_delta_log_sigma) + ")", "update", "CMAAdaptSigmaCSA", es.countiter, es.opts["verbose"])
...
class CMAAdaptSigmaMedianImprovement(CMAAdaptSigmaBase):
    ...
    def update(self, es, **kwargs):
        ...
        if 11 < 3:
            ...
            zkendall = stats.kendalltau(list(es.fit.fit) + list(self.fit), len(es.fit.fit) * [0] + len(self.fit) * [1])[0]
...
class CMAAdaptSigmaTPA(CMAAdaptSigmaBase):
    ...
    def update(self, es, function_values, **kwargs):
        ...
        if self.initialized is not True:
            utils.print_warning("dimension not known, damping set to 4", "update", "CMAAdaptSigmaTPA")
            self.initialized = True
        if 1 < 3:
    ...
    def check_consistency(self, es):
        ...
        if es.countiter > 3:
            ...
            for i in np.random.randint(0, es.N, 1):
                if i == j:
                    i -= 1 if i > 0 else -1
                if dx0 * dx1 * (es.pop[0][i] - es.mean[i]) * (es.pop[1][i] - es.mean[i]):
                    dmi_div_dx0i = (es.mean_after_tell[i] - es.mean_old[i]) / (es.pop[0][i] - es.mean[i])
                    dmi_div_dx1i = (es.mean_after_tell[i] - es.mean_old[i]) / (es.pop[1][i] - es.mean[i])
                    if not Mh.equals_approximately(dmi_div_dx0i, dm / dx0, 1e-4) or not Mh.equals_approximately(dmi_div_dx1i, dm / dx1, 1e-4):
                        utils.print_warning("TPA: apparent inconsistency with mirrored" " samples, where dmi_div_dx0i, dm/dx0=%f, %f" " and dmi_div_dx1i, dm/dx1=%f, %f" % (dmi_div_dx0i, dm / dx0, dmi_div_dx1i, dm / dx1), "check_consistency", "CMAAdaptSigmaTPA", es.countiter)
                else:
                    utils.print_warning("zero delta encountered in TPA which" + " \nshould be very rare and might be a bug" + " (sigma=%f)" % es.sigma, "check_consistency", "CMAAdaptSigmaTPA", es.countiter)
