...
try:
    np.median([1, 2, 3, 2])  # fails currently in pypy, also sigma_vec.scaling
...
class FitnessFunctions(object):  # TODO: this class is not necessary anymore? But some effort is needed to change it
    ...
    try:
        BBOB.__doc__ = bbobbenchmarks.__doc__
    except:
    ...
    def elli(self, x, rot=0, xoffset=0, cond=1e6, actuator_noise=0.0, both=False):
        ...
        ftrue = sum(cond ** (np.arange(N) / (N - 1.0)) * (x + xoffset) ** 2) if N > 1 else (x + xoffset) ** 2

        alpha = 0.49 + 1.0 / N
        beta = 1
        felli = np.random.rand(1)[0] ** beta * ftrue * max(1, (10.0**9 / (ftrue + 1e-99)) ** (alpha * np.random.rand(1)[0]))
    ...
    def grad_elli(self, x, *args):
        cond = 1e6
        N = len(x)
        return 2 * cond ** (np.arange(N) / (N - 1.0)) * array(x, copy=False)
    ...
    def rosen_chained(self, x, alpha=1e2):
        x = [x] if isscalar(x[0]) else x  # scalar into list
        f = [(1.0 - x[0]) ** 2 + sum(alpha * (x[:-1] ** 2 - x[1:]) ** 2) for x in x]
        return f if len(f) > 1 else f[0]  # 1-element-list into scalar
    ...
    def goldsteinprice(self, x):
        x1 = x[0]
        x2 = x[1]
        return (1 + (x1 + x2 + 1) ** 2 * (19 - 14 * x1 + 3 * x1**2 - 14 * x2 + 6 * x1 * x2 + 3 * x2**2)) * (30 + (2 * x1 - 3 * x2) ** 2 * (18 - 32 * x1 + 12 * x1**2 + 48 * x2 - 36 * x1 * x2 + 27 * x2**2)) - 3

    def griewank(self, x):
        # was in [-600 600]
        x = (600.0 / 5) * x
        return 1 - np.prod(np.cos(x / np.sqrt(1.0 + np.arange(len(x))))) + sum(x**2) / 4e3
    ...
    def schwefelmult(self, x, pen_fac=1e4):
        ...
        N = len(y[0])
        f = array([418.9829 * N - 1.27275661e-5 * N - sum(x * np.sin(np.abs(x) ** 0.5)) + pen_fac * sum((abs(x) > 500) * (abs(x) - 500) ** 2) for x in y])
        return f if len(f) > 1 else f[0]
    ...
    def powel_singular(self, x):
        ...
        res = np.sum((x[i - 1] + 10 * x[i]) ** 2 + 5 * (x[i + 1] - x[i + 2]) ** 2 + (x[i] - 2 * x[i + 1]) ** 4 + 10 * (x[i - 1] - x[i + 2]) ** 4 for i in range(1, len(x) - 2))
        return 1 + res
    ...
    def _fetch_bbob_fcts(self):
        ...
        print("extracting bbobbenchmarks.py")
        sys.stdout.flush()
        utils.extract_targz(url.split(os.path.sep)[-1], os.path.join("bbob.v" + bbob_version, "python", "bbobbenchmarks.py"))
