...
def unif(N, inseed):
    ...
    for i in xrange(int(N)):
        ...
        if aktseed < 0:
            aktseed = aktseed + 2147483647.0
        tmp = int(floor(aktrand / 67108865.0))
        aktrand = rgrand[tmp]
        rgrand[tmp] = aktseed
...
_randomnseed = 30.0  # warning this is a global variable...
...
_randomseed = 30.0  # warning this is a global variable...
...
def fUniform(ftrue, alpha, beta):
    ...
    # expects ftrue to be a np.array
    popsi = np.shape(ftrue)
    fval = _rand(popsi) ** beta * ftrue * np.maximum(1.0, (1e9 / (ftrue + 1e-99)) ** (alpha * _rand(popsi)))
    tol = 1e-8
    fval = fval + 1.01 * tol
    idx = ftrue < tol
    try:
...
def fCauchy(ftrue, alpha, p):
    ...
    # expects ftrue to be a np.array
    popsi = np.shape(ftrue)
    fval = ftrue + alpha * np.maximum(0.0, 1e3 + (_rand(popsi) < p) * _randn(popsi) / (np.abs(_randn(popsi)) + 1e-199))
    tol = 1e-8
    fval = fval + 1.01 * tol
    idx = ftrue < tol
    try:
...
class BBOBFunction(AbstractTestFunction):
    ...
    def __eq__(self, obj):
        return self.funId == obj.funId and (not hasattr(self, "param") or self.param == obj.param)
...
class BBOBGaussFunction(BBOBFunction):
    ...
    def boundaryhandling(self, x):
        return defaultboundaryhandling(x, 100.0)
...
class BBOBUniformFunction(BBOBFunction, object):
    ...
    def noise(self, ftrue):
        ...
        return fUniform(ftrue, self.unifalphafac * (0.49 + 1.0 / self.dim), self.unifbeta)
...
class _FStepEllipsoid(BBOBFunction):
    ...
    def initwithsize(self, curshape, dim):
        ...
        if self.dim != dim:
            ...
            self.linearTF = dot(compute_rotation(self.rseed, dim), diag(((self.condition / 10.0) ** 0.5) ** linspace(0, 1, dim)))
...
class _FRosenbrock(BBOBFunction):
    ...
    def initwithsize(self, curshape, dim):
        ...
        if self.dim != dim:
            if self.zerox:
                ...
                self.xopt = 0.75 * compute_xopt(self.rseed, dim)  # different from all others
    ...
    def _evalfull(self, x):
        ...
        # COMPUTATION core
        try:
            ftrue = 1e2 * np.sum((x[:, :-1] ** 2 - x[:, 1:]) ** 2, -1) + np.sum((x[:, :-1] - 1.0) ** 2, -1)
        except IndexError:
            ftrue = 1e2 * np.sum((x[:-1] ** 2 - x[1:]) ** 2) + np.sum((x[:-1] - 1.0) ** 2)
        fval = self.noise(ftrue)
...
class F9(BBOBNfreeFunction):
    ...
    def initwithsize(self, curshape, dim):
        ...
        if self.dim != dim:
            ...
            self.xopt = np.hstack(dot(0.5 * np.ones((1, dim)), self.linearTF.T)) / scale**2
...
class _FDiffPow(BBOBFunction):
    ...
    def initwithsize(self, curshape, dim):
        ...
        if self.lastshape != curshape:
            ...
            self.lastshape = curshape
            self.arrxopt = resize(self.xopt, curshape)
            self.arrexpo = resize(2.0 + self.alpha * linspace(0, 1, dim), curshape)
...
class F15(BBOBNfreeFunction):
    ...
    def _evalfull(self, x):
        ...
        x = monotoneTFosc(x)
        idx = x > 0.0
        x[idx] = x[idx] ** (1.0 + self.arrexpo[idx] * np.sqrt(x[idx]))  # smooth in zero
        x = dot(x, self.linearTF)
...
class F16(BBOBNfreeFunction):
    ...
    def _evalfull(self, x):
        ...
        ftrue = 10.0 * (ftrue / dim - self.f0) ** 3
        try:
            ftrue = np.hstack(ftrue)
        except TypeError:
            pass
...
class F17(_FSchaffersF7, BBOBNfreeFunction):
    ...
    def boundaryhandling(self, x):
        return defaultboundaryhandling(x, 10.0)
...
class _F8F2(BBOBFunction):
    ...
    def _evalfull(self, x):
        ...
        # COMPUTATION core
        try:
            f2 = 100.0 * (x[:, :-1] ** 2 - x[:, 1:]) ** 2 + (1.0 - x[:, :-1]) ** 2
        except IndexError:
            f2 = 100.0 * (x[:-1] ** 2 - x[1:]) ** 2 + (1.0 - x[:-1]) ** 2
        ftrue = self.facftrue + self.facftrue * np.sum(f2 / 4000.0 - np.cos(f2), -1) / (dim - 1.0)
        fval = self.noise(ftrue)
...
class F20(BBOBNfreeFunction):
    ...
    def _evalfull(self, x):
        ...
        try:
            x[:, 1:] = x[:, 1:] + 0.25 * (x[:, :-1] - self.arrxopt[:, :-1])
        except IndexError:
            x[1:] = x[1:] + 0.25 * (x[:-1] - self.arrxopt[:-1])
        x = 100.0 * (self.arrscales * (x - self.arrxopt) + self.arrxopt)
...
class _FGallagher(BBOBFunction):
    ...
    def initwithsize(self, curshape, dim):
        ...
        if self.dim != dim:
            ...
            self.peakvalues = np.insert(linspace(self.fitvalues[0], self.fitvalues[1], self.nhighpeaks - 1), 0, 10.0)

        # DIM- and POPSI-dependent initialisations of DIM*POPSI matrices
        if self.lastshape != curshape:
            self.dim = dim
            self.lastshape = curshape
            self.xlocal = dot(self.fac2 * np.reshape(10.0 * unif(dim * self.nhighpeaks, self.rseed) - 5.0, (self.nhighpeaks, dim)), self.rotation)
            if self.zerox:
...
class F23(BBOBNfreeFunction):
    ...
    funId = 23
    condition = 100.0
    arr2k = np.reshape(2.0 ** (np.arange(1, 33)), (1, 32))  # bug-fix for 32-bit (NH): 2 -> 2. (relevance is minor)
    ...
    def _evalfull(self, x):
        ...
        if len(curshape) < 2:  # popsize is 1 in this case
            ...
            ftrue = -10.0 / dim**2.0 + 10.0 / dim**2.0 * np.prod(1 + np.arange(1, dim + 1) * np.dot(np.abs(arr - np.round(arr)), self.arr2k.T**-1.0).T) ** (10.0 / dim**1.2)
        else:
            ftrue = zeros(curshape[0])
            for k, e in enumerate(x):
                arr = dot(np.reshape(e, (dim, 1)), self.arr2k)  # dim times d array
                ftrue[k] = -10.0 / dim**2.0 + 10.0 / dim**2.0 * np.prod(1 + np.arange(1, dim + 1) * np.dot(np.abs(arr - np.round(arr)), self.arr2k.T**-1.0).T) ** (10.0 / dim**1.2)
        fval = self.noise(ftrue)
...
class F24(BBOBNfreeFunction):
    ...
    def initwithsize(self, curshape, dim):
        ...
        if self.lastshape != curshape:
            ...
            self.lastshape = curshape
            # self.arrxopt = resize(self.xopt, curshape)
            self.arrscales = resize(2.0 * sign(self.xopt), curshape)  # makes up for xopt
    ...
    def _evalfull(self, x):
        ...
        # COMPUTATION core
        s = 1 - 0.5 / ((dim + 20) ** 0.5 - 4.1)  # tested up to DIM = 160 p in [0.25,0.33]
        d = 1  # shift [1,3], smaller is more difficult
        mu2 = -(((self._mu1**2 - d) / s) ** 0.5)
        ftrue = np.minimum(np.sum((x - self._mu1) ** 2, -1), d * dim + s * np.sum((x - mu2) ** 2, -1))
...
nfreefunclasses = (F1, F2, F3, F4, F5, F6, F7, F8, F9, F10, F11, F12, F13, F14, F15, F16, F17, F18, F19, F20, F21, F22, F23, F24)  # hard coded
noisyfunclasses = (F101, F102, F103, F104, F105, F106, F107, F108, F109, F110, F111, F112, F113, F114, F115, F116, F117, F118, F119, F120, F121, F122, F123, F124, F125, F126, F127, F128, F129, F130)
